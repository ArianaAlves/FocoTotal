...
# Wait for port 3000 to listen (max 30s)
...
if (-not $ready) {
  Write-Warning "Back-end did not respond on /health within timeout, but may still be starting."
} else {
  Write-Host "✅ Back-end running: http://localhost:3000/health"
}

Write-Host "🖥️  Starting front-end (Vite @ 5173)..."
Start-Job -ScriptBlock {
